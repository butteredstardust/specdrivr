...
export function TerminalLog({
  ...
  const converter = useMemo(() => new AnsiToHtml({
    fg: '#f4f4f5',
    bg: '#0d0d0a',
    escapeXML: true
  }), []);
  ...
  return (
    <div
      ...
      style={{ height }}
      className={twMerge(
        'terminal-surface overflow-y-auto bg-[--terminal-bg] text-[13px] font-mono p-2 relative rounded-md',
        className
      )}
    >
      {lines.length === 0 ? (
        ...
        <div className="flex flex-col gap-0.5 relative z-10">
          {lines.map((line) => {
            ...
            return (
              ...
              >
                <span className="text-[--text-muted] mr-3 shrink-0 select-none">
                  {time}
                </span>
                <span
