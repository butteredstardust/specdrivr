    ...
    <div className="flex flex-col gap-[var(--sp-4)]">
      ...
      {showFilters && (
        <div className="bg-[var(--bg-sunken)] border border-[var(--border-default)] rounded-[var(--radius-lg)] p-[var(--sp-5)] space-y-[var(--sp-4)] animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-[var(--sp-6)]">
            <div>
              <label className="block text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider mb-[var(--sp-2)]">Log Levels</label>
                ...
                {(Object.keys(levelFilters) as LogLevel[]).map((level) => (
                  <button
                    ...
                    className={cn(
                      ...
                      levelFilters[level]
                        ? cn("border-[var(--border-focus)]", levelLozengeStyles[level])
                        : "bg-[var(--bg-surface)] border-[var(--border-default)] text-[var(--text-tertiary)] opacity-50"
                    )}
                  >
                    {level}
                  </button>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider mb-[var(--sp-2)]">Related Task</label>
              <select
                value={selectedTaskId}
                onChange={(e) => setSelectedTaskId(e.target.value)}
                className="w-full h-[32px] bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[13px] px-[var(--sp-2)] focus:border-[var(--border-focus)] outline-none"
              >
            ...
            <div>
              <label className="block text-[11px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider mb-[var(--sp-2)]">Date Range</label>
              <div className="flex items-center gap-[var(--sp-2)]">
                <input
                  type="date"
                  value={dateFrom}
                  onChange={(e) => setDateFrom(e.target.value)}
                  className="w-full h-[32px] bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[12px] px-[var(--sp-2)]"
                />
      ...
      <div className="space-y-[var(--sp-2)]">
        {filteredLogs.length === 0 ? (
          <div className="py-[var(--sp-12)] flex flex-col items-center justify-center border-2 border-dashed border-[var(--border-default)] rounded-[var(--radius-lg)] opacity-60">
            <Search size={32} className="text-[var(--border-default)] mb-[var(--sp-3)]" />
            <p className="text-[14px] text-[var(--text-secondary)] italic">No logs found matching your filters.</p>
          </div>
        ) : (
          paginatedLogs.map((log) => {
            ...
            return (
              <div
                key={log.id}
                className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-sm)] p-[var(--sp-3)] hover:border-[var(--border-focus)] transition-colors group"
              >
                <div className="flex items-start gap-[var(--sp-4)]">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-[var(--sp-3)] mb-1">
                      ...
                      <div className="flex items-center gap-1 text-[11px] font-bold text-[var(--text-tertiary)] group-hover:text-[var(--brand-primary)] transition-colors">
                        <Clock size={10} />
                        {new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • SD-{log.taskId || 'SYSTEM'}
                      </div>
                    </div>
                    <p className="text-[14px] text-[var(--text-primary)] leading-relaxed">{log.message}</p>

                    {!!log.context && typeof log.context === 'object' && Object.keys(log.context).length > 0 && (
                      <details className="mt-[var(--sp-2)] group/details">
                        <summary className="text-[12px] font-bold text-[var(--brand-primary)] cursor-pointer hover:underline list-none flex items-center gap-1">
                          <Terminal size={12} />
                          View Execution Context
                        </summary>
                        <pre className="mt-[var(--sp-2)] p-[var(--sp-3)] bg-[var(--bg-sunken)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[11px] font-mono overflow-x-auto linear-scrollbar">
