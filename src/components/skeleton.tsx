...
export function Skeleton({
  ...
  return (
    <div
      className={cn('animate-pulse rounded-[var(--radius-sm)] bg-[var(--bg-sunken)]', className)}
      {...props}
    />
  );
}
  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div key={rowIndex} className="flex gap-4 p-2 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-sm)]">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {['To Do', 'In Progress', 'Done', 'Blocked'].map((status) => (
        <div key={status} className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-sm)] p-4 h-96">
