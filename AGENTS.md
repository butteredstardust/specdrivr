...
**TypeScript & Types** 4. Using `as Type` instead of type guards. 5. Duplicating Zod/Drizzle types manually instead of using `z.infer` / `$inferSelect`. 6. Introducing `any` types. 7. Omitting explicit return types. 8. Ignoring strict null checks (`null`/`undefined`). 9. Using default imports instead of named imports (`import type` for type-only).

**Database** 10. Direct `db` calls in components; bypassing repositories. 11. Multi-step writes without transaction. 12. Using raw SQL (`db.execute` or template literals). 13. Not using `executeQuery` in repository methods. 14. Editing or deleting files in `drizzle/` migrations. 15. Confusing `relations()` (query API) with FK constraints (schema).

**Security & Auth** 16. Forgetting `await auth()` before protected actions. 17. Calling `await auth()` after another `await` that could throw — always call it first. 18. Checking only global user role, ignoring project membership. 19. Logging sensitive data (passwords, tokens) via Pino. 20. Hardcoding secrets in code. 21. Missing rate limiting on public endpoints. 22. Forgetting to log audit events in same transaction. 23. Using `auth.api.getSession` in server components instead of the `auth()` helper. 24. Using `dangerouslySetInnerHTML` without `DOMPurify.sanitize()`.

**API & Validation** 25. Skipping Zod validation on inputs. 26. Returning raw error objects to client. 27. Using `pages/api` (Pages Router) instead of App Router. 28. Incorrect HTTP status codes (using 200 for errors). 29. Not awaiting `params`, `searchParams`, `cookies`, `headers` in Next.js 16. 30. Using Route Handlers for simple button clicks (use Server Actions instead).
