...
function verify() {
  ...
  if (Object.keys(expectedChecksums).length === 0) {
    console.error(
      '❌ No checksums found. Run "node scripts/verify-hooks.js generate" first.',
    );
    process.exit(1);
  }
  ...
  const actualFiles = criticalHooks.filter((hook) =>
    existsSync(path.join(hookDir, hook)),
  );
  ...
  if (failed) {
    ...
    console.error(
      "  1. Legitimate hook updates (run generate to update checksums)",
    );
...
function checkGitConfig() {
  ...
  try {
    ...
    if (hooksPath && hooksPath !== ".husky" && hooksPath !== ".husky/_") {
      console.error(
        `  ✗ WARNING: git core.hooksPath is set to '${hooksPath}' instead of '.husky'`,
      );
  ...
  try {
    ...
    if (templateDir) {
      ...
      console.error(
        "    This could bypass hooks. Run: git config --unset init.templateDir",
      );
      issues++;
    } else {
  ...
  if (!existsSync(hookDir)) {
    console.error(
      `  ✗ CRITICAL: .husky directory does not exist at ${hookDir}`,
    );
    issues++;
  } else {
