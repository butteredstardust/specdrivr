...
# Rule: XSS Prevention
# Detect dangerouslySetInnerHTML without DOMPurify.sanitize().
...
for file in $react_files; do
    ...
    # We look for lines containing dangerouslySetInnerHTML but NOT DOMPurify.sanitize
    violations=$(grep -n "dangerouslySetInnerHTML" "$file" | grep -v "DOMPurify\.sanitize" || true)
    ...
    if [ -n "$violations" ]; then
        ...
        echo "    Requirement: All HTML injected via dangerouslySetInnerHTML must be sanitized using 'DOMPurify.sanitize()'."
        FAILED=1
    fi
done
