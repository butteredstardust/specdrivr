...
function logPreCommit(user, status, details = '') {
  ...
  const logFile = path.join(auditDir, `pre-commit-${new Date().toISOString().split('T')[0]}.log`);
  appendFileSync(logFile, JSON.stringify(logEntry) + '\n');

  // Log to console if hook was bypassed
  if (status === 'bypassed' && bypassMethod !== 'none') {
    console.error('⚠️  WARNING: Pre-commit checks were bypassed with --no-verify');
    console.error('   This usage is logged for security purposes.');
  }

  if (configBypass !== 'none') {
    console.error(`⚠️  WARNING: Git config has bypass settings: ${configBypass}`);
  }
}
...
function logPrePush(user, status, details = '', bypassMethodArg = '') {
  ...
  const logFile = path.join(auditDir, `pre-push-${new Date().toISOString().split('T')[0]}.log`);
  appendFileSync(logFile, JSON.stringify(logEntry) + '\n');

  // Log to console if hook was bypassed
  if (status === 'bypassed' && bypassMethod !== 'none') {
    console.error('⚠️  WARNING: Pre-push checks were bypassed with --no-verify');
    console.error('   This usage is logged for security purposes.');
  }
    ...
    if (existsSync(file)) {
      ...
      if (lines.length > 0 && lines[0] !== '') {
        lines.slice(-5).forEach((line) => {
          ...
          console.log(`  ${entry.timestamp} | ${entry.user} | branch=${entry.branch} | ${status}`);
        });
      } else {
