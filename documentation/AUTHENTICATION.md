...
- Session storage: Managed by Better Auth with database persistence (Postgres via Drizzle adapter).
- Cookies: Uses `better-auth.session_token` (and `__Secure-` prefix in production). `cookieCache` is enabled for speed (5m).
...
- API tokens: generated as `sdk_{projectSlug}_{48 random hex chars}`. Stored as `bcrypt` hash. Verification uses prefix-based lookup (first 10 chars) for performance. Shown to user exactly once on creation.
