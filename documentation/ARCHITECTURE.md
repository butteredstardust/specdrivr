...
| Client-side data fetching waterfalls: using `useEffect` or `usePolling` on mount without initial data causes unnecessary loading states and violates React 19 server-first paradigms. | Refactor to Server-Side Initial Fetch: fetch data in the Server Component and pass it as `initialData` to client hooks, or use React 19 `use(promise)` with `<Suspense>` for streaming. |
...
| Repository evasion / complex queries: putting complex multi-table Drizzle joins (aggregations, counts) into standard `BaseRepository` subclasses bloats the CRUD repositories. | Establish strict separation: standard Repositories (`FooRepository extends BaseRepository`) are for simple CRUD. Complex cross-table read models must be placed in dedicated Query Objects in `src/queries/`. |
