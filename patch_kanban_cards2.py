...
content = content.replace('className="text-[11px] font-bold text-[var(--color-text-tertiary)] uppercase leading-none"',
                          'className="text-[11px] font-[400] text-[var(--text-tertiary)] font-mono leading-none"')

# Update Title text color & size
content = content.replace('className="text-[14px] text-[var(--color-text-primary)] leading-tight mb-[var(--sp-3)] line-clamp-2 group-hover:text-[var(--color-brand-bold)] transition-colors"',
                          'className="text-[13px] font-[500] text-[var(--text-primary)] leading-tight mb-[var(--sp-3)] line-clamp-3 transition-colors"')

# Update status lozenge class
content = content.replace('className={cn(\n              "px-[6px] py-[2px] rounded-[3px] text-[10px] font-bold uppercase tracking-wider",\n              taskStatusColors[status as keyof typeof taskStatusColors]?.bg || "bg-[var(--status-todo-bg)]",\n              taskStatusColors[status as keyof typeof taskStatusColors]?.text || "text-[var(--status-todo-text)]"\n            )}',
